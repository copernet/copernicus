...
func WriteMessage(w io.Writer, message Message, pver uint32, net utils.BitcoinNet) (int, error) {
	totalBytes := 0
	var command [CommandSize]byte
	cmd := message.Command()
...
func ReadMessage(reader io.Reader, pver uint32, bitcoinNet utils.BitcoinNet) (int, Message, []byte, error) {
	totalBytes := 0

...
func (msg *MsgVersion) Decode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	t := int64(0)
	err := util.ReadElements(buf, &msg.ProtocolVersion, (*uint64)(&msg.Services), &t)
	if err != nil {
		return err
	}
	msg.Timestamp = time.Unix(t, 0)
	...
	if buf.Len() > 0 {
		err = util.ReadElements(buf, &msg.Nonce)
		if err != nil {
			return err
		}
	}
	if buf.Len() > 0 {
		userAgent, err := util.ReadVarString(buf)
		if err != nil {
			return err
		}
	...
	if buf.Len() > 0 {
		err = util.ReadElements(buf, &msg.LastBlock)
		if err != nil {
			return err
		}
	}
...
func (msg *MsgVersion) Encode(w io.Writer, pver uint32, enc MessageEncoding) error {
	...
	err = util.WriteElements(w, msg.ProtocolVersion, uint64(msg.Services),
		msg.Timestamp.Unix())
	if err != nil {
		return err
	}
	...
	err = util.WriteElements(w, msg.Nonce)
	if err != nil {
		return err
	}

	err = util.WriteVarString(w, msg.UserAgent)
	if err != nil {
		return err
	}

	err = util.WriteElements(w, msg.LastBlock)
	if err != nil {
		return err
	}
	...
	if pver >= BIP0037Version {
		err = util.WriteElements(w, !msg.DisableRelayTx)
		if err != nil {
			return err
		}
	}

...
func TestElementWire(t *testing.T) {
	...
	tests := []struct {
		...
		{
			(*util.Hash)(&[util.Hash256Size]byte{ // Make go vet happy.
				0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
				0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10,
	...
	for i, test := range tests {
		...
		err := util.WriteElements(&buf, test.in)
		if err != nil {
		...
		err = util.ReadElements(rbuf, val)
		if err != nil {
...
func TestElementWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := util.WriteElements(w, test.in)
...
func TestVarIntWire(t *testing.T) {
	...
	for i, test := range tests {
		// Encode to wire format.
		var buf bytes.Buffer
		err := util.WriteVarInt(&buf, test.in)
		if err != nil {
...
func TestVarIntWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		// Encode to wire format.
		w := newFixedWriter(test.max)
		err := util.WriteVarInt(w, test.in)
		if err != test.writeErr {
...
func TestVarIntNonCanonical(t *testing.T) {
	...
	for i, test := range tests {
		...
		val, err := util.ReadVarInt(rbuf)
		if err == nil {
			t.Errorf("expted readvarint error")
		}
		/*
...
func TestVarStringWire(t *testing.T) {
	...
	for i, test := range tests {
		// Encode to wire format.
		var buf bytes.Buffer
		err := util.WriteVarString(&buf, test.in)
		if err != nil {
		...
		// Decode from wire format.
		rbuf := bytes.NewReader(test.buf)
		val, err := util.ReadVarString(rbuf)
		if err != nil {
...
func TestVarStringWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		// Encode to wire format.
		w := newFixedWriter(test.max)
		err := util.WriteVarString(w, test.in)
		if err != test.writeErr {
...
func TestVarStringOverflowErrors(t *testing.T) {
	...
	for _, test := range tests {
		// Decode from wire format.
		rbuf := bytes.NewReader(test.buf)
		_, err := util.ReadVarString(rbuf)
		if err == nil {
			t.Errorf("expect ReadVarString error")
		}
		/*
...
func TestVarBytesWire(t *testing.T) {
	...
	for i, test := range tests {
		// Encode to wire format.
		var buf bytes.Buffer
		err := util.WriteVarBytes(&buf, test.in)
		if err != nil {
		...
		// Decode from wire format.
		rbuf := bytes.NewReader(test.buf)
		val, err := util.ReadVarBytes(rbuf, MaxMessagePayload,
			"test payload")
		if err != nil {
...
func TestVarBytesWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		// Encode to wire format.
		w := newFixedWriter(test.max)
		err := util.WriteVarBytes(w, test.in)
		if err != test.writeErr {
		...
		// Decode from wire format.
		r := newFixedReader(test.max, test.buf)
		_, err = util.ReadVarBytes(r, MaxMessagePayload,
			"test payload")
		if err != test.readErr {
...
func TestVarBytesOverflowErrors(t *testing.T) {
	...
	for _, test := range tests {
		// Decode from wire format.
		rbuf := bytes.NewReader(test.buf)
		_, err := util.ReadVarBytes(rbuf, MaxMessagePayload,
			"test payload")
		if err == nil {
			t.Errorf("expect ReadVarBytes error")
		}
		/*
			if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
...
func TestRandomUint64Errors(t *testing.T) {
	...
	nonce, err := util.GetUint64FromReder(fr)

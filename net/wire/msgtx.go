...
func (msg *MsgTx) TxHash() util.Hash {
...
func (msg *MsgTx) WitnessHash() util.Hash {
	if msg.HasWitness() {
...
func (msg *MsgTx) BtcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	version, err := util.BinarySerializer.Uint32(r, littleEndian)
	if err != nil {
		return err
	}
	...
	if flag[0] != 0 && enc == WitnessEncoding {
		for _, txin := range msg.TxIn {
			...
			witCount, err := util.ReadVarInt(r)
			if err != nil {
				returnScriptBuffers()
				return err
			}
	...
	msg.LockTime, err = util.BinarySerializer.Uint32(r, littleEndian)
	if err != nil {
		returnScriptBuffers()
		return err
	}
...
func (msg *MsgTx) BtcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	err := util.BinarySerializer.PutUint32(w, littleEndian, uint32(msg.Version))
	if err != nil {
		return err
	}
	...
	return util.BinarySerializer.PutUint32(w, littleEndian, msg.LockTime)
}
...
func readOutPoint(r io.Reader, pver uint32, version int32, op *OutPoint) error {
	...
	op.Index, err = util.BinarySerializer.Uint32(r, littleEndian)
	return err
}
...
func writeOutPoint(w io.Writer, pver uint32, version int32, op *OutPoint) error {
	...
	return util.BinarySerializer.PutUint32(w, littleEndian, op.Index)
}
...
func readTxIn(r io.Reader, pver uint32, version int32, ti *TxIn) error {
	...
	return util.ReadElements(r, &ti.Sequence)
}
...
func writeTxIn(w io.Writer, pver uint32, version int32, ti *TxIn) error {
	...
	err = util.WriteVarBytes(w, ti.SignatureScript)
	if err != nil {
		return err
	}

	return util.BinarySerializer.PutUint32(w, littleEndian, ti.Sequence)
}

// readTxOut reads the next sequence of bytes from r as a transaction output
// (TxOut).
func readTxOut(r io.Reader, pver uint32, version int32, to *TxOut) error {
	err := util.ReadElements(r, &to.Value)
	if err != nil {
		return err
	}
...
func WriteTxOut(w io.Writer, pver uint32, version int32, to *TxOut) error {
	err := util.BinarySerializer.PutUint64(w, littleEndian, uint64(to.Value))
	if err != nil {
		return err
	}

	return util.WriteVarBytes(w, to.PkScript)
}

// writeTxWitness encodes the bitcoin protocol encoding for a transaction
// input's witness into to w.
func writeTxWitness(w io.Writer, pver uint32, version int32, wit [][]byte) error {
	err := util.WriteVarInt(w, uint64(len(wit)))
	if err != nil {
		return err
	}
	for _, item := range wit {
		err = util.WriteVarBytes(w, item)
		if err != nil {
			return err
		}
	}

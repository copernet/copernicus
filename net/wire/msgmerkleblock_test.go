...
func TestMerkleBlock(t *testing.T) {
	...
	for i := 0; i < maxTxPerBlock; i++ {
		rand.Read(data)
		hash, err := util.BytesToHash(data)
		if err != nil {
			t.Errorf("BytesToHash failed: %v\n", err)
			return
		}
...
func TestMerkleBlockOverflowErrors(t *testing.T) {
	...
	// allowed tx hashes.
	var buf bytes.Buffer
	util.WriteVarInt(&buf, maxTxPerBlock+1)
	numHashesOffset := 84
	...
	// allowed flag bytes.
	buf.Reset()
	util.WriteVarInt(&buf, maxFlagsPerMerkleBlock+1)
...
var merkleBlockOne = MsgMerkleBlock{
	Header: BlockHeader{
		Version: 1,
		PrevBlock: util.Hash([util.Hash256Size]byte{ // Make go vet happy.
			0x6f, 0xe2, 0x8c, 0x0a, 0xb6, 0xf1, 0xb3, 0x72,
			0xc1, 0xa6, 0xa2, 0x46, 0xae, 0x63, 0xf7, 0x4f,
			0x93, 0x1e, 0x83, 0x65, 0xe1, 0x5a, 0x08, 0x9c,
			0x68, 0xd6, 0x19, 0x00, 0x00, 0x00, 0x00, 0x00,
		}),
		MerkleRoot: util.Hash([util.Hash256Size]byte{ // Make go vet happy.

...
func (msg *MsgFilterLoad) Decode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	var err error
	msg.Filter, err = util.ReadVarBytes(r, MaxFilterLoadFilterSize,
		"filterload filter size")
	if err != nil {
		return err
	}

	err = util.ReadElements(r, &msg.HashFuncs, &msg.Tweak, &msg.Flags)
	if err != nil {
		return err
	}
...
func (msg *MsgFilterLoad) Encode(w io.Writer, pver uint32, enc MessageEncoding) error {
	...
	return util.WriteElements(w, msg.HashFuncs, msg.Tweak, msg.Flags)
}

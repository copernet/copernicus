...
func (msg *MsgFeeFilter) Decode(r io.Reader, pver uint32, enc MessageEncoding) error {
	if pver < FeeFilterVersion {
		str := fmt.Sprintf("feefilter message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgFeeFilter.Decode", str)
	}

	return util.ReadElements(r, &msg.MinFee)
}
...
func (msg *MsgFeeFilter) Encode(w io.Writer, pver uint32, enc MessageEncoding) error {
	if pver < FeeFilterVersion {
		str := fmt.Sprintf("feefilter message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgFeeFilter.Encode", str)
	}

	return util.WriteElements(w, msg.MinFee)
}

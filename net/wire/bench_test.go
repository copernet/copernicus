...
func BenchmarkWriteVarInt1(b *testing.B) {
	for i := 0; i < b.N; i++ {
		util.WriteVarInt(ioutil.Discard, 1)
	}
}
...
func BenchmarkWriteVarInt3(b *testing.B) {
	for i := 0; i < b.N; i++ {
		util.WriteVarInt(ioutil.Discard, 65535)
	}
}
...
func BenchmarkWriteVarInt5(b *testing.B) {
	for i := 0; i < b.N; i++ {
		util.WriteVarInt(ioutil.Discard, 4294967295)
	}
}
...
func BenchmarkWriteVarInt9(b *testing.B) {
	for i := 0; i < b.N; i++ {
		util.WriteVarInt(ioutil.Discard, 18446744073709551615)
	}
}
...
func BenchmarkWriteVarStr4(b *testing.B) {
	for i := 0; i < b.N; i++ {
		util.WriteVarString(ioutil.Discard, "test")
	}
}
...
func BenchmarkWriteVarStr10(b *testing.B) {
	for i := 0; i < b.N; i++ {
		util.WriteVarString(ioutil.Discard, "test012345")
	}
}
...
// BenchmarkUnserializeTx performs a benchmark on how long it takes to
// deserialize a small transaction.
func BenchmarkUnserializeTxSmall(b *testing.B) {
	buf := []byte{
...
// BenchmarkUnserializeTxLarge performs a benchmark on how long it takes to
// deserialize a very large transaction.
func BenchmarkUnserializeTxLarge(b *testing.B) {
...
func BenchmarkDecodeGetHeaders(b *testing.B) {
	...
	for i := 0; i < MaxBlockLocatorsPerMsg; i++ {
		hash, err := util.GetHashFromStr(fmt.Sprintf("%x", i))
		if err != nil {
	...
	var bb bytes.Buffer
	if err := m.Encode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgGetHeaders.Encode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
	...
	for i := 0; i < b.N; i++ {
		r.Seek(0, 0)
		msg.Decode(r, pver, LatestEncoding)
	}
}
...
func BenchmarkDecodeHeaders(b *testing.B) {
	...
	var bb bytes.Buffer
	if err := m.Encode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgHeaders.Encode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
...
func BenchmarkDecodeGetBlocks(b *testing.B) {
	...
	var bb bytes.Buffer
	if err := m.Encode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgGetBlocks.Encode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
...
func BenchmarkDecodeAddr(b *testing.B) {
	...
	// Serialize it so the bytes are available to test the decode below.
	var bb bytes.Buffer
	if err := ma.Encode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgAddr.Encode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
...
func BenchmarkDecodeInv(b *testing.B) {
	...
	var bb bytes.Buffer
	if err := m.Encode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgInv.Encode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
...
func BenchmarkDecodeNotFound(b *testing.B) {
	...
	var bb bytes.Buffer
	if err := m.Encode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgNotFound.Encode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
...
func BenchmarkDecodeMerkleBlock(b *testing.B) {
	...
	hash, err := util.GetHashFromStr(fmt.Sprintf("%x", 10000))
	if err != nil {
		b.Fatalf("NewHashFromStr: unexpected error: %v", err)
	}
	...
	var bb bytes.Buffer
	if err := m.Encode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgMerkleBlock.Encode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
...
func BenchmarkDoubleHashB(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = util.DoubleSha256Bytes(txBytes)
	}
}
...
func BenchmarkDoubleHashH(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = util.DoubleSha256Hash(txBytes)
	}
}
*/

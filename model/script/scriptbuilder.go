...
func (b *ScriptBuilder) addData(data []byte) *ScriptBuilder {
	...
	if dataLen == 0 || dataLen == 1 && data[0] == 0 {
		b.script = append(b.script, opcodes.OP_0)
		return b
	} else if dataLen == 1 && data[0] <= 16 {
		b.script = append(b.script, (opcodes.OP_1-1)+data[0])
		return b
	} else if dataLen == 1 && data[0] == 0x81 {
		b.script = append(b.script, byte(opcodes.OP_1NEGATE))
		return b
	}
	...
	if dataLen < opcodes.OP_PUSHDATA1 {
		b.script = append(b.script, byte((opcodes.OP_DATA_1-1)+dataLen))
	} else if dataLen <= 0xff {
		b.script = append(b.script, opcodes.OP_PUSHDATA1, byte(dataLen))
	} else if dataLen <= 0xffff {
		buf := make([]byte, 2)
		binary.LittleEndian.PutUint16(buf, uint16(dataLen))
		b.script = append(b.script, opcodes.OP_PUSHDATA2)
		b.script = append(b.script, buf...)
	} else {
		buf := make([]byte, 4)
		binary.LittleEndian.PutUint32(buf, uint32(dataLen))
		b.script = append(b.script, opcodes.OP_PUSHDATA4)
		b.script = append(b.script, buf...)
	}
...
func (b *ScriptBuilder) AddInt64(val int64) *ScriptBuilder {
	...
	if val == -1 || (val >= 1 && val <= 16) {
		b.script = append(b.script, byte((opcodes.OP_1-1)+val))
		return b
	}

	return b.AddData(NewScriptNum(val).Bytes())
}

...
func TestScriptBuilderAddOp(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "push OP_0",
			opcodes:  []byte{opcodes.OP_0},
			expected: []byte{opcodes.OP_0},
		},
		{
			name:     "push OP_1 OP_2",
			opcodes:  []byte{opcodes.OP_1, opcodes.OP_2},
			expected: []byte{opcodes.OP_1, opcodes.OP_2},
		},
		{
			name:     "push OP_HASH160 OP_EQUAL",
			opcodes:  []byte{opcodes.OP_HASH160, opcodes.OP_EQUAL},
			expected: []byte{opcodes.OP_HASH160, opcodes.OP_EQUAL},
		},
	}
...
func TestScriptBuilderAddInt64(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "push -1", val: -1, expected: []byte{opcodes.OP_1NEGATE}},
		{name: "push small int 0", val: 0, expected: []byte{opcodes.OP_0}},
		{name: "push small int 1", val: 1, expected: []byte{opcodes.OP_1}},
		{name: "push small int 2", val: 2, expected: []byte{opcodes.OP_2}},
		{name: "push small int 3", val: 3, expected: []byte{opcodes.OP_3}},
		{name: "push small int 4", val: 4, expected: []byte{opcodes.OP_4}},
		{name: "push small int 5", val: 5, expected: []byte{opcodes.OP_5}},
		{name: "push small int 6", val: 6, expected: []byte{opcodes.OP_6}},
		{name: "push small int 7", val: 7, expected: []byte{opcodes.OP_7}},
		{name: "push small int 8", val: 8, expected: []byte{opcodes.OP_8}},
		{name: "push small int 9", val: 9, expected: []byte{opcodes.OP_9}},
		{name: "push small int 10", val: 10, expected: []byte{opcodes.OP_10}},
		{name: "push small int 11", val: 11, expected: []byte{opcodes.OP_11}},
		{name: "push small int 12", val: 12, expected: []byte{opcodes.OP_12}},
		{name: "push small int 13", val: 13, expected: []byte{opcodes.OP_13}},
		{name: "push small int 14", val: 14, expected: []byte{opcodes.OP_14}},
		{name: "push small int 15", val: 15, expected: []byte{opcodes.OP_15}},
		{name: "push small int 16", val: 16, expected: []byte{opcodes.OP_16}},
		{name: "push 17", val: 17, expected: []byte{opcodes.OP_DATA_1, 0x11}},
		{name: "push 65", val: 65, expected: []byte{opcodes.OP_DATA_1, 0x41}},
		{name: "push 127", val: 127, expected: []byte{opcodes.OP_DATA_1, 0x7f}},
		...
		{name: "push -2", val: -2, expected: []byte{opcodes.OP_DATA_1, 0x82}},
		{name: "push -3", val: -3, expected: []byte{opcodes.OP_DATA_1, 0x83}},
		{name: "push -4", val: -4, expected: []byte{opcodes.OP_DATA_1, 0x84}},
		{name: "push -5", val: -5, expected: []byte{opcodes.OP_DATA_1, 0x85}},
		{name: "push -17", val: -17, expected: []byte{opcodes.OP_DATA_1, 0x91}},
		{name: "push -65", val: -65, expected: []byte{opcodes.OP_DATA_1, 0xc1}},
		{name: "push -127", val: -127, expected: []byte{opcodes.OP_DATA_1, 0xff}},
		//{name: "push -128", val: -128, expected: []byte{opcodes.OP_DATA_2, 0x80, 0x80}},
...
func TestScriptBuilderAddData(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "push empty byte sequence", data: nil, expected: []byte{opcodes.OP_0}},
		{name: "push 1 byte 0x00", data: []byte{0x00}, expected: []byte{opcodes.OP_0}},

		// BIP0062: Pushing a 1-byte sequence of byte 0x01 through 0x10 must use OP_n.
		{name: "push 1 byte 0x01", data: []byte{0x01}, expected: []byte{opcodes.OP_1}},
		{name: "push 1 byte 0x02", data: []byte{0x02}, expected: []byte{opcodes.OP_2}},
		{name: "push 1 byte 0x03", data: []byte{0x03}, expected: []byte{opcodes.OP_3}},
		{name: "push 1 byte 0x04", data: []byte{0x04}, expected: []byte{opcodes.OP_4}},
		{name: "push 1 byte 0x05", data: []byte{0x05}, expected: []byte{opcodes.OP_5}},
		{name: "push 1 byte 0x06", data: []byte{0x06}, expected: []byte{opcodes.OP_6}},
		{name: "push 1 byte 0x07", data: []byte{0x07}, expected: []byte{opcodes.OP_7}},
		{name: "push 1 byte 0x08", data: []byte{0x08}, expected: []byte{opcodes.OP_8}},
		{name: "push 1 byte 0x09", data: []byte{0x09}, expected: []byte{opcodes.OP_9}},
		{name: "push 1 byte 0x0a", data: []byte{0x0a}, expected: []byte{opcodes.OP_10}},
		{name: "push 1 byte 0x0b", data: []byte{0x0b}, expected: []byte{opcodes.OP_11}},
		{name: "push 1 byte 0x0c", data: []byte{0x0c}, expected: []byte{opcodes.OP_12}},
		{name: "push 1 byte 0x0d", data: []byte{0x0d}, expected: []byte{opcodes.OP_13}},
		{name: "push 1 byte 0x0e", data: []byte{0x0e}, expected: []byte{opcodes.OP_14}},
		{name: "push 1 byte 0x0f", data: []byte{0x0f}, expected: []byte{opcodes.OP_15}},
		{name: "push 1 byte 0x10", data: []byte{0x10}, expected: []byte{opcodes.OP_16}},

		// BIP0062: Pushing the byte 0x81 must use OP_1NEGATE.
		{name: "push 1 byte 0x81", data: []byte{0x81}, expected: []byte{opcodes.OP_1NEGATE}},
		...
		{name: "push 1 byte 0x11", data: []byte{0x11}, expected: []byte{opcodes.OP_DATA_1, 0x11}},
		{name: "push 1 byte 0x80", data: []byte{0x80}, expected: []byte{opcodes.OP_DATA_1, 0x80}},
		{name: "push 1 byte 0x82", data: []byte{0x82}, expected: []byte{opcodes.OP_DATA_1, 0x82}},
		{name: "push 1 byte 0xff", data: []byte{0xff}, expected: []byte{opcodes.OP_DATA_1, 0xff}},
		//{
		//	name:     "push data len 17",
		//	data:     bytes.Repeat([]byte{0x49}, 17),
		//	expected: append([]byte{opcodes.OP_DATA_17}, bytes.Repeat([]byte{0x49}, 17)...),
		//},
		//{
		//	name:     "push data len 75",
		//	data:     bytes.Repeat([]byte{0x49}, 75),
		//	expected: append([]byte{opcodes.OP_DATA_75}, bytes.Repeat([]byte{0x49}, 75)...),
		//},
		...
		// BIP0062: Pushing 76 to 255 bytes must use OP_PUSHDATA1.
		{
			...
			expected: append([]byte{opcodes.OP_PUSHDATA1, 76}, bytes.Repeat([]byte{0x49}, 76)...),
		},
		{
			name:     "push data len 255",
			data:     bytes.Repeat([]byte{0x49}, 255),
			expected: append([]byte{opcodes.OP_PUSHDATA1, 255}, bytes.Repeat([]byte{0x49}, 255)...),
		},
		...
		// BIP0062: Pushing 256 to 520 bytes must use OP_PUSHDATA2.
		{
			...
			expected: append([]byte{opcodes.OP_PUSHDATA2, 0, 1}, bytes.Repeat([]byte{0x49}, 256)...),
		},
		{
			name:     "push data len 520",
			data:     bytes.Repeat([]byte{0x49}, 520),
			expected: append([]byte{opcodes.OP_PUSHDATA2, 0x08, 0x02}, bytes.Repeat([]byte{0x49}, 520)...),
		},
		...
		// 3-byte data push via OP_PUSHDATA_2.
		{
			...
			data:     bytes.Repeat([]byte{0x49}, 32767),
			expected: append([]byte{opcodes.OP_PUSHDATA2, 255, 127}, bytes.Repeat([]byte{0x49}, 32767)...),
			useFull:  true,
		},
		...
		// 5-byte data push via OP_PUSHDATA_4.
		{
			...
			data:     bytes.Repeat([]byte{0x49}, 65536),
			expected: append([]byte{opcodes.OP_PUSHDATA4, 0, 0, 1, 0}, bytes.Repeat([]byte{0x49}, 65536)...),
			useFull:  true,
		},
	}
...
func TestExceedMaxScriptSize(t *testing.T) {
	...
	builder.Reset().AddFullData(make([]byte, MaxScriptSize-3))
	script, err = builder.AddOp(opcodes.OP_0).Script()
	if _, ok := err.(ErrScriptNotCanonical); !ok || err == nil {

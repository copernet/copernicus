...
func VerifyScript(tx *Tx, index int, scriptSig *Script, scriptPubKey *Script, flags uint32, err *core.ScriptError) bool {
	...
	if ip.IsEmpty() {
		return SetError(err, core.SCRIPT_ERR_EVAL_FALSE)
	}
	...
	if flags&core.SCRIPT_VERIFY_P2SH != 0 && scriptPubKey.IsPayToScriptHash() {
		...
		// Restore stack
		if copyIP != nil {
			algorithm.SwapStack(ip.stack, copyIP.stack)
		}
	...
	if flags&core.SCRIPT_VERIFY_CLEANSTACK != 0 {
		...
		if ip.stack.Size() != 1 {
			return SetError(err, core.SCRIPT_ERR_CLEANSTACK)
		}
	}
...
func SetError(ret *core.ScriptError, seterror core.ScriptError) bool {
	if ret != nil {
		*ret = seterror
	}
	return false
...
func SetSuccess(ret *core.ScriptError) bool {
	if ret != nil {
		*ret = core.SCRIPT_ERR_OK
	}
	return true
}

...
func (s *RPCServer) WebsocketHandler(conn *websocket.Conn, remoteAddr string,
	...
	if err != nil {
		logs.Error("Failed to serve client %s: %v", remoteAddr, err)
		conn.Close()
		return
	}
	...
	logs.Info("Disconnected websocket client %s", remoteAddr)
}
...
out:
	for {
		...
		case n, ok := <-m.notificationMsgs:
			...
			default:
				logs.Warn("Unhandled notification type")
			}
...
func (*wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient,
	...
	if err != nil {
		logs.Error("Failed to marshal block connected notification: "+
			"%v", err)
		return
	}
...
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *btcutil.Block) {
	...
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
		logs.Error("Failed to marshal block disconnected "+
			"notification: %v", err)
		return
	}
	for _, wsc := range clients {
...
func (m *wsNotificationManager) notifyFilteredBlockConnected(clients map[chan struct{}]*wsClient,
	...
	if err != nil {
		logs.Error("Failed to serialize header for filtered block "+
	...
	for quitChan, wsc := range clients {
		...
		marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
		if err != nil {
			logs.Error("Failed to marshal filtered block "+
				"connected notification: %v", err)
			return
		}
		wsc.QueueNotification(marshalledJSON)
...
func (*wsNotificationManager) notifyFilteredBlockDisconnected(clients map[chan struct{}]*wsClient,
	...
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
		logs.Error("Failed to marshal filtered block disconnected "+
			"notification: %v", err)
		return
	}
	for _, wsc := range clients {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *btcutil.Tx) {
	...
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
		logs.Error("Failed to marshal tx notification: %s", err.Error())
		return
	}
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			marshalledJSONVerbose, err = btcjson.MarshalCmd(nil,
				verboseNtfn)
			if err != nil {
				logs.Error("Failed to marshal verbose tx "+
...
func (*wsNotificationManager) removeSpentRequest(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	notifyMap, ok := ops[*op]
	if !ok {
		logs.Warn("Attempt to remove nonexistent spent request "+
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	for i, txOut := range tx.MsgTx().TxOut {
		...
		for _, txAddr := range txAddrs {
			...
			marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
			if err != nil {
				logs.Error("Failed to marshal processedtx notification: %v", err)
				continue
			}
...
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *btcutil.Tx,
	...
	if len(clientsToNotify) != 0 {
		...
		if err != nil {
			logs.Error("Failed to marshal notification: %v", err)
			return
		}
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	for _, txIn := range tx.MsgTx().TxIn {
		...
		if cmap, ok := ops[*prevOut]; ok {
			...
			if err != nil {
				logs.Warn("Failed to marshal redeemingtx notification: %v", err)
				continue
			}
			for wscQuit, wsc := range cmap {
				if block != nil {
...
func (*wsNotificationManager) removeAddrRequest(addrs map[string]map[chan struct{}]*wsClient,
	...
	cmap, ok := addrs[addr]
	if !ok {
		logs.Warn("Attempt to remove nonexistent addr request "+
...
func (m *wsNotificationManager) Start() {
	m.wg.Add(2)
	go m.queueHandler()
	//go m.notificationHandler() // todo open
}
...
out:
	for {
		...
		if err != nil {
			...
			if err != io.EOF {
				logs.Error("Websocket receive error from "+
					"%s: %v", c.addr, err)
			}
			break out
		}
		...
		if err != nil {
			...
			if err != nil {
				logs.Error("Failed to marshal parse failure "+
					"reply: %v", err)
				continue
			}
		...
		logs.Debug("Received command <%s> from %s", cmd.method, c.addr)
		...
		case c.authenticated && ok:
			logs.Warn("Websocket client %s is already authenticated",
				c.addr)
			break out
		case !c.authenticated && !ok:
			logs.Warn("Unauthenticated websocket message " +
				"received")
			break out
		case !c.authenticated:
			...
			if err != nil {
				logs.Error("Failed to marshal authenticate reply: "+
					"%v", err.Error())
				continue
			}
			c.SendMessage(reply, nil)
	...
	c.Disconnect()
	c.wg.Done()
	logs.Trace("Websocket client input handler done for %s", c.addr)
}
...
func (c *wsClient) serviceRequest(r *parsedRPCCmd) {
	...
	if err != nil {
		logs.Error("Failed to marshal reply for <%s> "+
...
cleanup:
	...
	c.wg.Done()
	logs.Trace("Websocket client notification queue handler done "+
		"for %s", c.addr)
}
...
cleanup:
	...
	c.wg.Done()
	logs.Trace("Websocket client output handler done for %s", c.addr)
}
...
func (c *wsClient) Disconnect() {
	...
	logs.Trace("Disconnecting websocket client %s", c.addr)
	close(c.quit)
	c.conn.Close()
	c.disconnected = true
}
...
func (c *wsClient) Start() {
	logs.Trace("Starting websocket client %s", c.addr)
	...
	for _, tx := range blk.Transactions() {
		...
		for _, txin := range tx.MsgTx().TxIn {
			if _, ok := lookups.unspent[txin.PreviousOutPoint]; ok {
				...
				if err != nil {
					logs.Error("Failed to marshal redeemingtx notification: %v", err)
					continue
				}
		...
		for txOutIdx, txout := range tx.MsgTx().TxOut {
			...
			for _, addr := range addrs {
				...
				case *btcutil.AddressPubKey:
					...
					default:
						logs.Warn("Skipping rescanned pubkey of unknown "+
				...
				marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
				if err != nil {
					logs.Error("Failed to marshal recvtx notification: %v", err)
					return
				}
...
func recoverFromReorg(chain *blockchain.BlockChain, minBlock, maxBlock int32,
	...
	if err != nil {
		logs.Error("Error looking up block range: %v", err)
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCDatabase,
	...
	if err != nil {
		logs.Error("Error looking up possibly reorged block: %v",
			err)
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCDatabase,
			Message: "Database error: " + err.Error(),
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *btcutil.Block) error {
	...
	if !prevHash.IsEqual(curHash) {
		logs.Error("Stopping rescan for reorged block %v "+
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	if numAddrs == 1 {
		logs.Info("Beginning rescan for 1 address")
	} else {
		logs.Info("Beginning rescan for %d addresses", numAddrs)
	}
...
fetchRange:
	for minBlock < maxBlock {
		...
		if len(hashList) == 0 {
			...
			close(pauseGuard)
			if err != nil {
				logs.Error("Error fetching best block "+
					"hash: %v", err)
				return nil, &btcjson.RPCError{
					Code: btcjson.ErrRPCDatabase,
	...
	loopHashList:
		for i := range hashList {
			...
			if err != nil {
				...
				if maxBlock != math.MaxInt32 {
					logs.Error("Stopping rescan for "+
						"reorged block %v",
						cmd.EndBlock)
			...
			select {
			case <-wsc.quit:
				logs.Debug("Stopped rescan at height %v "+
			...
			if err != nil {
				logs.Error("Failed to marshal rescan "+
	...
	if mn, err := btcjson.MarshalCmd(nil, n); err != nil {
		logs.Error("Failed to marshal rescan finished "+
			"notification: %v", err)
	} else {

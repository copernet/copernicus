...
func (r FutureGenerateResult) Receive() ([]*utils.Hash, error) {
	...
	// Convert each block hash to a utils.Hash and store a pointer to
	// each.
	convertedResult := make([]*utils.Hash, len(result))
	for i, hashString := range result {
		convertedResult[i], err = utils.GetHashFromStr(hashString)
		if err != nil {
			return nil, err
		}
	}
...
func (c *Client) Generate(numBlocks uint32) ([]*utils.Hash, error) {

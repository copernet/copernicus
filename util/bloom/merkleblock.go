...
import (
	"github.com/copernicus/logic/merkleroot"
	"github.com/copernicus/model/block"
...
// node position.
func (m *merkleBlock) calcHash(height, pos uint32) *util.Hash {
	if height == 0 {
...
func NewMerkleBlock(block *block.Block, filter *Filter) (*wire.MsgMerkleBlock, []uint32) {
	numTx := uint32(len(block.Txs))
	mBlock := merkleBlock{
		numTx:       numTx,
		allHashes:   make([]*util.Hash, 0, numTx),
	...
	for txIndex, tx := range block.Txs {
		...
		hash := tx.GetHash()
		mBlock.allHashes = append(mBlock.allHashes, &hash)
	}
	...
	msgMerkleBlock := wire.MsgMerkleBlock{
		...
		Hashes:       make([]*util.Hash, 0, len(mBlock.finalHashes)),
